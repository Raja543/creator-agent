...
export async function GET(request: NextRequest) {
  ...
  let query = supabase.from("events").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(limit);
...
export async function POST(request: NextRequest) {
  ...
  if (error) return errResponse(error);
  await logActivity(user.id, "event_detected", `New event detected: ${body.title}`, { event_id: data.id, ecosystem: body.ecosystem });
